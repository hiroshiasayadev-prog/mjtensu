...
- 2026-08-28 target-device debug evidence later showed that the original transitive connected-component duplicate rule can collapse several distinct meld detections through one large merged box. PRODUCT-TASK-SYSTEM-002-12 owns the correction and current `spec:product.recognition.pipeline` is the authority for merged-bridge rejection plus non-transitive pairwise duplicate resolution. This completed Task remains evidence of the original implementation, not authority for the corrected algorithm.
