...
from ..study.definition import StudyExistingModelSource, StudyTrainingModelSource
...
def _not_found(kind: EntityKind, entity_id: str) -> NotFoundError:
    return NotFoundError(f'{kind.value} {entity_id!r} not found')


def _dependency_issue(exc: Exception) -> ValidationReport:
    return ValidationReport((ValidationIssue(
        code='definition.dependency.invalid',
...
def _resolve_report(
    ...
    tracked_filesystem: _TrackingFilesystem | None = None,
) -> tuple[object | None, ValidationReport]:
    try:
        return call(), ValidationReport()
    except ValidationFailedError as exc:
        if tracked_filesystem is not None and tracked_filesystem.failure is not None:
            raise tracked_filesystem.failure
        return None, exc.report
    except (NotFoundError, InvalidRequestError, LifecycleConflictError) as exc:
        if tracked_filesystem is not None and tracked_filesystem.failure is not None:
            raise tracked_filesystem.failure
        return None, _dependency_issue(exc)


def _result(kind: ReusableDefinitionKind, entity_id: ReusableDefinitionId, report: ValidationReport) -> DefinitionValidationResult:
    return DefinitionValidationResult(kind=kind, id=entity_id, report=report)


def _hash_issue(expected: str | None, actual: str, path: str) -> ValidationIssue | None:
    if expected == actual:
        return None
    return ValidationIssue(
        code='definition.implementation.sha256_mismatch',
        message='Current implementation bytes do not match recorded implementation.sha256.',
        path=path,
    )
...
def validate_task_definition(
    ...
    from ..runtime.resolution import resolve_task

    if not filesystem.file_exists(layout.task_metadata_path(task_id)):
        raise _not_found(EntityKind.TASK, task_id)
    tracked = _TrackingFilesystem(filesystem)
    _, report = _resolve_report(lambda: resolve_task(task_id, layout, tracked), tracked)
    return _result(EntityKind.TASK, task_id, report)
...
def validate_corpus_definition(
    ...
    from ..runtime.resolution import resolve_corpus

    if not filesystem.file_exists(layout.corpus_metadata_path(corpus_id)):
        raise _not_found(EntityKind.CORPUS, corpus_id)
    tracked = _TrackingFilesystem(filesystem)
    _, report = _resolve_report(lambda: resolve_corpus(corpus_id, layout, tracked), tracked)
    return _result(EntityKind.CORPUS, corpus_id, report)
...
def validate_architecture_definition(
    ...
    if not filesystem.file_exists(layout.architecture_metadata_path(architecture_id)):
        raise _not_found(EntityKind.ARCHITECTURE, architecture_id)
    tracked = _TrackingFilesystem(filesystem)
    handle, report = _resolve_report(lambda: resolve_architecture(architecture_id, layout, tracked), tracked)
    if handle is None:
        return _result(EntityKind.ARCHITECTURE, architecture_id, report)
    issues = list(report.issues)
    if handle.metadata.status is ArchitectureStatus.SEALED:
        actual = hashlib.sha256(filesystem.read_bytes(handle.implementation_path)).hexdigest()
        issue = _hash_issue(handle.metadata.implementation.sha256, actual, 'implementation.sha256')
        if issue is not None:
            issues.append(issue)
    return _result(EntityKind.ARCHITECTURE, architecture_id, ValidationReport(tuple(issues)))
...
def validate_train_protocol_definition(
    ...
    if not filesystem.file_exists(layout.train_protocol_metadata_path(protocol_id)):
        raise _not_found(EntityKind.TRAIN_PROTOCOL, protocol_id)
    tracked = _TrackingFilesystem(filesystem)
    handle, report = _resolve_report(lambda: resolve_train_protocol(protocol_id, layout, tracked), tracked)
    if handle is None:
        return _result(EntityKind.TRAIN_PROTOCOL, protocol_id, report)
    issues = list(report.issues)
    if handle.metadata.status is TrainProtocolStatus.SEALED:
        actual = hashlib.sha256(filesystem.read_bytes(handle.implementation_path)).hexdigest()
        issue = _hash_issue(handle.metadata.implementation.sha256, actual, 'implementation.sha256')
        if issue is not None:
            issues.append(issue)
    return _result(EntityKind.TRAIN_PROTOCOL, protocol_id, ValidationReport(tuple(issues)))
...
def validate_evaluation_protocol_definition(
    ...
    if not filesystem.file_exists(layout.evaluation_protocol_metadata_path(protocol_id)):
        raise _not_found(EntityKind.EVALUATION_PROTOCOL, protocol_id)
    tracked = _TrackingFilesystem(filesystem)
    handle, report = _resolve_report(lambda: resolve_evaluation_protocol(protocol_id, layout, tracked), tracked)
    if handle is None:
        return _result(EntityKind.EVALUATION_PROTOCOL, protocol_id, report)
    issues = list(report.issues)
    if handle.metadata.status is EvaluationProtocolStatus.SEALED:
        actual = hashlib.sha256(filesystem.read_bytes(handle.implementation_path)).hexdigest()
        issue = _hash_issue(handle.metadata.implementation.sha256, actual, 'implementation.sha256')
        if issue is not None:
            issues.append(issue)
    return _result(EntityKind.EVALUATION_PROTOCOL, protocol_id, ValidationReport(tuple(issues)))
...
def validate_study_definition(
    ...
    if not filesystem.file_exists(layout.study_metadata_path(study_id)):
        raise _not_found(EntityKind.STUDY, study_id)
    tracked = _TrackingFilesystem(filesystem)
    study_handle, report = _resolve_report(lambda: resolve_study(study_id, layout, tracked), tracked)
    if study_handle is None:
        return _result(EntityKind.STUDY, study_id, report)

    study = study_handle.metadata
    issues: list[ValidationIssue] = list(report.issues)
    common_task = None

    def require(call):
        nonlocal issues
        value, dependency_report = _resolve_report(call, tracked)
        if value is None:
            issues.extend(dependency_report.issues)
        return value

    if isinstance(study.model, StudyTrainingModelSource):
        corpus = require(lambda: resolve_corpus(study.model.corpus, layout, tracked))
        protocol = require(lambda: resolve_train_protocol(study.model.protocol, layout, tracked))
        if corpus is not None:
            common_task = corpus.metadata.task
        if protocol is not None and common_task is not None and protocol.metadata.task != common_task:
            issues.append(ValidationIssue('study.task_mismatch', 'Train Protocol Task differs from the Study common Task.', 'model.train.protocol'))
        for index, architecture_id in enumerate(study.model.architectures):
            architecture = require(lambda architecture_id=architecture_id: resolve_architecture(architecture_id, layout, tracked))
            if architecture is not None and common_task is not None and architecture.metadata.task != common_task:
                issues.append(ValidationIssue('study.task_mismatch', 'Architecture Task differs from the Study common Task.', f'model.train.architectures[{index}]'))
        if protocol is not None:
            for key in sorted(study.model.parameters):
                if key not in protocol.metadata.parameters:
                    issues.append(ValidationIssue('study.parameter.unknown', f'Train Protocol does not publish parameter {key!r}.', f'model.train.parameters[{key!r}]'))
    elif isinstance(study.model, StudyExistingModelSource):
        for index, model_id in enumerate(study.model.models):
            model = require(lambda model_id=model_id: resolve_model(model_id, layout, tracked))
            if model is None:
                continue
            model_task = model.architecture.metadata.task
            if common_task is None:
                common_task = model_task
            elif model_task != common_task:
                issues.append(ValidationIssue('study.task_mismatch', 'Existing Model Task differs from the Study common Task.', f'model.existing[{index}]'))

    for index, stage in enumerate(study.evaluations):
        corpus = require(lambda stage=stage: resolve_corpus(stage.corpus, layout, tracked))
        protocol = require(lambda stage=stage: resolve_evaluation_protocol(stage.protocol, layout, tracked))
        if corpus is not None:
            if common_task is None:
                common_task = corpus.metadata.task
            elif corpus.metadata.task != common_task:
                issues.append(ValidationIssue('study.task_mismatch', 'Evaluation Corpus Task differs from the Study common Task.', f'evaluations[{index}].corpus'))
        if protocol is not None:
            if common_task is None:
                common_task = protocol.metadata.task
            elif protocol.metadata.task != common_task:
                issues.append(ValidationIssue('study.task_mismatch', 'Evaluation Protocol Task differs from the Study common Task.', f'evaluations[{index}].protocol'))
            for key in sorted(stage.parameters):
                if key not in protocol.metadata.parameters:
                    issues.append(ValidationIssue('study.parameter.unknown', f'Evaluation Protocol does not publish parameter {key!r}.', f'evaluations[{index}].parameters[{key!r}]'))

    return _result(EntityKind.STUDY, study_id, ValidationReport(tuple(issues)))
