...
Later classifier work retained the same `[N,1,64,64] -> [N,35]` contract while changing the implementation architecture. PRODUCT-INV-RECOGNITION-011 first selected standard MobileNetV3-Small 1.0x as a production replacement candidate after dense-angle evaluation and direct iPhone 13 ONNX Runtime Web A/B measurement, but live production-pipeline use exposed practical fine-grained tile-identity regressions. PRODUCT-INV-RECOGNITION-012 then selected the resolution-preserving `mobile-tile-f8-r1.onnx` finalist, which preserves an `8 x 8` late feature map and retains a measured iPhone latency advantage over Plain. The current runtime spec remains architecture-neutral (`gray64-tile-35-v1`) rather than naming any concrete backbone.
