...
- Target-device re-execution is still required before this Task can return to `done`: verify portrait-locked entry, physically hold the device in landscape, confirm camera motion matches device motion without a 90-degree phase error or stretch, and confirm `反対向き` keeps preview/Recognition geometry aligned.
