...
def project_training_assignment(
    ...
    _validate_training_agreement(preflight, run, attempt)
    architecture_sha256 = preflight.architecture.metadata.implementation.sha256
    protocol_sha256 = preflight.protocol.metadata.implementation.sha256
    if architecture_sha256 is None:
        raise ValueError("Training Architecture implementation SHA-256 is missing.")
    if protocol_sha256 is None:
        raise ValueError("Train Protocol implementation SHA-256 is missing.")

    corpus_artifact = _describe_exact(
        assets,
        preflight.corpus.artifact_path,
        key="corpus_artifact",
        sha256=preflight.corpus.metadata.artifact.sha256,
        bytes=preflight.corpus.metadata.artifact.bytes,
    )
    architecture_implementation = _describe_exact(
        assets,
    ...
    train_protocol_implementation = _describe_exact(
        assets,
        preflight.protocol.implementation_path,
        key="train_protocol_implementation",
    ...
    return TrainingAssignment(
        ...
        kind="training",
        task=preflight.task.metadata,
        corpus=preflight.corpus.metadata,
        architecture=preflight.architecture.metadata,
        protocol=preflight.protocol.metadata,
        seed=preflight.seed,
        parameters=preflight.parameters,
        corpus_artifact=corpus_artifact,
        architecture_implementation=architecture_implementation,
        train_protocol_implementation=train_protocol_implementation,
    )
...
def project_evaluation_assignment(
    ...
    _validate_evaluation_agreement(preflight, run, attempt)
    architecture_sha256 = preflight.model.architecture.metadata.implementation.sha256
    protocol_sha256 = preflight.protocol.metadata.implementation.sha256
    training_result = preflight.model.training_run.result
    if architecture_sha256 is None:
        raise ValueError("Model Architecture implementation SHA-256 is missing.")
    if protocol_sha256 is None:
        raise ValueError("Evaluation Protocol implementation SHA-256 is missing.")
    if training_result is None:
        raise ValueError("Model lineage Training Run result is missing.")
    ...
    model_architecture_implementation = _describe_exact(
        assets,
        preflight.model.architecture.implementation_path,
        key="model_architecture_implementation",
        sha256=architecture_sha256,
        bytes=None,
    )
    weights = training_result.weights
    model_weights = _describe_exact(
        assets,
    ...
    evaluation_protocol_implementation = _describe_exact(
        assets,
        preflight.protocol.implementation_path,
        key="evaluation_protocol_implementation",
        sha256=protocol_sha256,
        bytes=None,
    )
    ...
    return EvaluationAssignment(
        ...
        model=preflight.model.metadata,
        training_run=preflight.model.training_run,
        model_architecture=preflight.model.architecture.metadata,
        protocol=preflight.protocol.metadata,
        parameters=preflight.parameters,
        corpus_artifact=corpus_artifact,
        model_architecture_implementation=model_architecture_implementation,
        model_weights=model_weights,
        evaluation_protocol_implementation=evaluation_protocol_implementation,
    )


def _validate_training_agreement(
    preflight: TrainingPreflight,
    run: TrainingRun,
    attempt: QueueAttempt,
) -> None:
    checks = (
        (run.status is TrainingRunStatus.RUNNING, "Training Run must be RUNNING."),
        (attempt.finished_at is None, "Queue attempt must be open."),
        (attempt.run_id == run.id, "Queue attempt Run ID does not match Training Run."),
        (run.corpus == preflight.corpus.metadata.id, "Training corpus identity mismatch."),
        (run.architecture == preflight.architecture.metadata.id, "Training Architecture identity mismatch."),
        (run.train_protocol == preflight.protocol.metadata.id, "Train Protocol identity mismatch."),
        (run.execution.seed == preflight.seed, "Training seed mismatch."),
        (run.parameters == preflight.parameters, "Training parameters mismatch."),
    )
    for agreed, message in checks:
        if not agreed:
            raise ValueError(message)


def _validate_evaluation_agreement(
    preflight: EvaluationPreflight,
    run: EvaluationRun,
    attempt: QueueAttempt,
) -> None:
    training_run = preflight.model.training_run
    checks = (
        (run.status is EvaluationRunStatus.RUNNING, "Evaluation Run must be RUNNING."),
        (attempt.finished_at is None, "Queue attempt must be open."),
        (attempt.run_id == run.id, "Queue attempt Run ID does not match Evaluation Run."),
        (run.model == preflight.model.metadata.id, "Evaluation Model identity mismatch."),
        (run.corpus == preflight.corpus.metadata.id, "Evaluation corpus identity mismatch."),
        (run.evaluation_protocol == preflight.protocol.metadata.id, "Evaluation Protocol identity mismatch."),
        (run.parameters == preflight.parameters, "Evaluation parameters mismatch."),
        (training_run.id == preflight.model.metadata.training_run, "Model Training Run lineage identity mismatch."),
        (training_run.status is TrainingRunStatus.COMPLETED, "Model lineage Training Run must be COMPLETED."),
        (training_run.result is not None, "Model lineage Training Run result is missing."),
        (training_run.architecture == preflight.model.architecture.metadata.id, "Model Architecture lineage identity mismatch."),
    )
    for agreed, message in checks:
        if not agreed:
            raise ValueError(message)
...
def _describe_exact(
    ...
    *,
    ...
    descriptor = assets.describe(
        source_path,
        key=key,
        sha256=sha256,
        bytes=bytes,
    )
    if not isinstance(descriptor, ImmutableAssetDescriptor):
        raise ValueError("Immutable asset source returned an invalid descriptor type.")
    if (
        descriptor.key != key
        or descriptor.sha256 != sha256
        or descriptor.bytes != bytes
    ):
        raise ValueError(
            f"Immutable asset descriptor does not preserve requested integrity for {key!r}."
        )
    return descriptor
