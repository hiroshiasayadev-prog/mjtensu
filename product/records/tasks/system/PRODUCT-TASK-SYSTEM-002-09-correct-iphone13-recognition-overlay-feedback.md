...
- F-MAJ-05 and F-MAJ-06 were therefore corrected in production code and automated verification completed for the first lightweight tile-face implementation. Real-device visual acceptance remains owned by PRODUCT-TASK-SYSTEM-002-04.
- Follow-up visual refinement replaces the CSS/text tile approximation with the vendored/generated CC0 tile SVG assets. Recognized observation identity is rendered as a `22×30` tile image centered immediately above the candidate bbox (`bottom: calc(100% + 2px)`), so the user-facing identity no longer covers the physical tile being aligned.
- Red fives resolve to dedicated `5m-red.svg` / `5p-red.svg` / `5s-red.svg` assets, while concealed-kan preview backs use the shared `back.svg` asset. Unresolved observations keep their distinct warning treatment.
- The tile SVG source set remains local and build-owned. Because the Vite configuration reserves `publicDir` for vendored Recognition model payloads, `src/ui/tile-assets.ts` uses static `new URL(..., import.meta.url)` references to include the generated `frontend/public/tiles` SVGs as normal Vite build assets without changing model serving behavior.
- Focused tests now additionally assert bbox-external identity placement, the expected tile SVG/red-five/back asset selection, and retained recognized/unresolved/meld differentiation.
- Follow-up automated verification completed on 2026-08-27: `npx vitest run test/recognition-page.test.tsx` — **PASS**, 11/11 tests; `npm run typecheck` — **PASS**; `npm run lint` — **PASS**, architecture import boundaries OK across 61 source files; `npm run build` — **PASS**, Vite 8.2.2 production PWA build completed.
- The production build emitted all generated tile SVGs and `back.svg` as hashed `dist/assets/*.svg` artifacts, confirming that the static `new URL(..., import.meta.url)` references are production-resolved rather than runtime external URLs.
- Workbox precache increased to 47 entries and includes the generated SVG assets, so the Recognition tile identities remain available from the installed/offline PWA shell after the updated worker activates.
