...
- The first separate-layout attempt still misinterpreted "same display" as identical CSS pixel dimensions across orientations. The resulting short-edge sizing (`min(100vw, 100dvh)`) made the landscape capture surface remain only portrait-width wide, leaving large black margins even though its internal `16:9` and bbox geometry were correct. Target-device screenshots made this regression obvious.
- The corrected layout interpretation is now: preserve the same `16:9` camera geometry and the same normalized semantic/bbox proportions, but fit that geometry appropriately to each viewport. Portrait uses a width-filled `100vw × 56.25vw` surface; landscape uses the largest fully-visible `16:9` surface (`min(100vw, 177.7778dvh)` × `min(100dvh, 56.25vw)`). Camera content, bbox aspect ratios, and normalized placement remain unchanged; only viewport fit differs.
...
- Focused tests now require the orientation-specific viewport-fit formulas while preserving identical semantic bbox percentage geometry across portrait/landscape resize, plus aspect-preserving center crop for portrait camera input.
...
- Automated verification completed on 2026-08-27 for the corrected orientation-specific viewport fit: `npx vitest run test/camera-service.test.ts test/recognition-page.test.tsx` — **PASS**, 18/18 tests; `npm run typecheck` — **PASS**; `npm run build` — **PASS**, Vite 8.2.2 production PWA build completed. Follow-up application bundle: `assets/index-CuXj7i8q.js`.
- Final requirement clarification: portrait viewport support exists only to tolerate iOS orientation lock. The user still physically holds the phone in landscape and must see the exact same landscape Recognition UI as normal landscape mode. There is no portrait-specific semantic layout.
- The implementation is therefore split into two coordinate layers. The outer camera surface follows the browser viewport: `16:9` in real landscape and native `9:16` while the browser remains portrait-locked. The camera preview is never CSS-rotated or stretched; it fills that outer surface with `object-fit: cover` so camera motion remains natural.
- All application-owned Recognition UI stays one `16:9` landscape surface using the original single `RECOGNITION_CAPTURE_REGIONS`: mask, dora/completed-hand/meld frames, observation bboxes, status, exit, and recovery controls. In a portrait-locked viewport this complete landscape UI surface is centered inside the `9:16` camera surface and rotated `90deg`; in real landscape it fills the camera surface without a transform.
- Recognition input applies the inverse transform so visible camera coordinates and landscape UI coordinates remain aligned. Portrait-locked capture first center-crops the native camera frame to `9:16`, then quarter-turns it `-90deg` into a canonical `16:9` frame before passing the unchanged landscape regions downstream. The UI applies `+90deg` to the same logical landscape coordinate system, so the two transforms cancel when bboxes are displayed over the unrotated camera preview. Real landscape capture remains `16:9` with rotation `0`.
