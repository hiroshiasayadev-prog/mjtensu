...
- Target-device re-execution is still required before this Task can return to `done`: verify portrait fills the available width, landscape expands to the largest fully-visible `16:9` surface, camera content is not stretched, semantic/bbox aspect ratios remain identical, and camera motion remains natural in both viewport orientations.
