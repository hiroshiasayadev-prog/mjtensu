...
- **F-MAJ-11 — stable meld rows are rejected by a redundant per-row fitted-angle gate**: debug capture `mjtensu-recognition-debug-2026-08-27T17-37-35-246Z.json` recognizes five meld tiles as `7m 8m 9m` and `1p 1p` but reports `unresolved-meld-geometry`. The existing projected-row clustering can form the intended `3 + 2` partition, while least-squares bbox-center fits produce approximately `-35.15°` and `-23.04°`. The runtime already limits candidate/common row directions to the required `±22.5°` support range and scores per-row angle residuals; separately rejecting every fitted row beyond `±22.5°` makes short rows overly sensitive to detector bbox-center jitter. Correction is routed to PRODUCT-TASK-SYSTEM-002-14.
