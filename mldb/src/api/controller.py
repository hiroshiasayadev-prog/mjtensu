...
def validate_definition(
    ...
    if kind is EntityKind.TASK:
        result = _validate_task_definition(TaskId(entity_id), layout, filesystem)
    elif kind is EntityKind.CORPUS:
        result = _validate_corpus_definition(CorpusId(entity_id), layout, filesystem)
    elif kind is EntityKind.ARCHITECTURE:
        result = _validate_architecture_definition(ArchitectureId(entity_id), layout, filesystem)
    elif kind is EntityKind.TRAIN_PROTOCOL:
        result = _validate_train_protocol_definition(TrainProtocolId(entity_id), layout, filesystem)
    elif kind is EntityKind.EVALUATION_PROTOCOL:
        result = _validate_evaluation_protocol_definition(EvaluationProtocolId(entity_id), layout, filesystem)
    elif kind is EntityKind.STUDY:
        result = _validate_study_definition(StudyId(entity_id), layout, filesystem)
    else:
        raise UnsupportedOperationError(f"validate_definition is not supported for {kind!r}")

    return DefinitionValidationResponse(
        kind=result.kind,
        id=result.id,
        valid=result.valid,
...
def seal_definition(
    ...
    if kind is EntityKind.ARCHITECTURE:
        if runner is None:
            raise InvalidRequestError("runner is required for Architecture sealing")
        typed_id = ArchitectureId(entity_id)
        result = _seal_architecture(typed_id, layout, filesystem, runner)
    elif kind is EntityKind.TRAIN_PROTOCOL:
        if runner is None:
            raise InvalidRequestError("runner is required for Train Protocol sealing")
        typed_id = TrainProtocolId(entity_id)
        result = _seal_train_protocol(typed_id, layout, filesystem, runner)
    elif kind is EntityKind.EVALUATION_PROTOCOL:
        if runner is None:
            raise InvalidRequestError("runner is required for Evaluation Protocol sealing")
        typed_id = EvaluationProtocolId(entity_id)
        result = _seal_evaluation_protocol(typed_id, layout, filesystem, runner)
    elif kind is EntityKind.STUDY:
        if runner is not None:
            raise InvalidRequestError("runner must not be supplied for Study sealing")
        typed_id = StudyId(entity_id)
        result = _seal_study(typed_id, layout, filesystem)
    else:
        raise UnsupportedOperationError(f"seal_definition is not supported for {kind!r}")

    return SealDefinitionResponse(
        kind=kind,
        id=typed_id,
        status="sealed",
        result=result,
...
def get_study_run(
    ...
    return _get_study_run_progress(
        study_run_id,
        as_of,
        layout,
        filesystem,
...
def cancel_study_run(
    ...
    return _request_study_run_cancellation(
        study_run_id,
        finished_at,
        queue_at,
        layout,
...
def get_entity(
    ...
    return _get_entity(kind, entity_id, layout, filesystem)
...
def list_entities(
    ...
    return _list_entities(kind, layout, filesystem)
