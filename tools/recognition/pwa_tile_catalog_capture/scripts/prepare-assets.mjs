...
const sourceModelPath = process.env.MJTENSU_TILE_CATALOG_MODEL
  ? resolve(process.env.MJTENSU_TILE_CATALOG_MODEL)
  : defaultModelPath;
const outputModelName = 'nanodet-plus-m-320-tile-catalog.onnx';
...
await copyFile(layoutSource, join(publicDirectory, 'tile_catalog_layout.v2.json'));
...
await writeFile(
  join(modelDirectory, 'nanodet-plus-m-320-tile-catalog.metadata.json'),
  `${JSON.stringify(metadata, null, 2)}\n`,
  'utf8',
);

console.log(`Prepared tile catalog model: ${outputModelPath}`);
