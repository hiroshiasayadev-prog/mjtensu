...
- Target-device re-execution is still required before this Task can return to `done`, specifically with iOS orientation lock enabled: open Recognition in portrait viewport, confirm realtime starts without orientation guidance, physically hold the device in landscape, and verify the rotated surface plus `反対向き` control are usable.
