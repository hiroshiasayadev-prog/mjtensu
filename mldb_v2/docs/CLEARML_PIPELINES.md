...
For multi-Model Evaluation, MLDB builds a Study-level comparison projection from accepted canonical Evaluation Results. The ClearML controller renders one compact comparison table and one fixed-height Model Comparison plot per Evaluation stage, with readable trial labels derived from Model/Architecture lineage. The Model Comparison plot shows one metric at a time and uses a Plotly dropdown to switch metrics, avoiding a tall subplot stack that would force every ClearML Plot card to the same large height. Evaluation Protocol metric `preference` controls comparison coloring: `higher` and `lower` map rank from blue through light-blue/light-red to red, while `neutral` or omitted preference keeps the normal Plotly color. Trial ordering is unchanged and the colors do not select a winning model. Controller Scalars are not used for this cross-sectional comparison; time-series Training/Evaluation telemetry remains on the child Tasks.
