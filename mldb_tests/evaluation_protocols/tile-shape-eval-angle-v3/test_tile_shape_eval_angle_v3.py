...
def test_evaluate_supports_cpu_cache_parameters(tmp_path: Path) -> None:
    ...
    protocol = resolve_evaluation_protocol(
        EvaluationProtocolId("tile-shape-eval-angle-v3"), layout, filesystem
    )
...
def test_evaluate_rejects_invalid_cache_fraction(tmp_path: Path) -> None:
    database = tmp_path / "fixture.sqlite"
    _database(database)
    layout = RepositoryLayout(REPO_ROOT)
    filesystem = LocalFilesystem()
