...
_ALLOWED_WORK_KINDS = frozenset({"training", "evaluation"})
...
class AcquireWorkRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.acquire_token, str) or not self.acquire_token:
            raise ValueError("acquire_token must be a non-empty string.")
        if not isinstance(self.accepts, frozenset) or not self.accepts:
            raise ValueError("accepts must be a non-empty frozenset.")
        if not self.accepts <= _ALLOWED_WORK_KINDS:
            raise ValueError("accepts may contain only 'training' and 'evaluation'.")
...
class ImmutableAssetDescriptor:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.key, str) or not self.key:
            raise ValueError("asset descriptor key must be a non-empty string.")
        if not isinstance(self.retrieval_id, str) or not self.retrieval_id:
            raise ValueError("asset descriptor retrieval_id must be a non-empty string.")
        if not isinstance(self.sha256, str) or _SHA256_RE.fullmatch(self.sha256) is None:
            raise ValueError("asset descriptor sha256 must be a 64-character hexadecimal digest.")
        if self.bytes is not None and (
            not isinstance(self.bytes, int) or isinstance(self.bytes, bool) or self.bytes < 0
        ):
            raise ValueError("asset descriptor bytes must be a non-negative integer or None.")
