...
- Meld-row tilt guidance is non-blocking. When the absolute selected common meld-row angle exceeds `30°` for three consecutive evaluated frames, show `牌の並びを水平にすると認識が安定します` or equivalent guidance. While that guidance is visible, clear it only after the absolute common angle remains below `25°` for three consecutive evaluated frames. A frame with no stable common angle resets the pending show counter when guidance is hidden, and resets the pending clear counter while guidance is visible; it does not itself hide active guidance.
- Meld-row tilt guidance must not alter grouping support, frame eligibility, semantic stabilization, or automatic commit. Recognition continues normally even while the guidance is visible; the `±45°` grouping support boundary remains independently defined by `spec:product.recognition.runtime_recognition`.
