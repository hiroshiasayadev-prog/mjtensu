...
def train(context):
    ...
    for epoch in range(epochs):
        ...
        context.telemetry.report_scalar(
            group="validation", series="f1", value=key[0], step=completed_epoch
        )
        context.telemetry.report_scalar(
            group="validation", series="recall", value=key[1], step=completed_epoch
        )
        context.telemetry.report_scalar(
            group="validation", series="mean_iou", value=key[2], step=completed_epoch
        )
        context.telemetry.report_scalar(
            group="validation", series="loss", value=-key[3], step=completed_epoch
        )
