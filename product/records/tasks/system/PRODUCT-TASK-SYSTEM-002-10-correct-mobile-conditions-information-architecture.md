...
- Follow-up iPhone rounded-corner protection on 2026-08-28 updates the shared mobile scoring shell to reserve horizontal safe-area padding for the app bar, scroll content, and persistent bottom bar using `max(..., env(safe-area-inset-left/right))`; the same shell fix is shared with I11 Result rather than duplicated per page.
