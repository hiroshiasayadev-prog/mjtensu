...
from .jobs import EvaluationJob as _EvaluationJob, TrainingJob as _TrainingJob
...
def handle_heartbeat(
    ...
    **Lease-authority serialization and races.** The required order is current Queue
    ...
    with _orchestration_exclusion():
        ...
        if attempt is None:
            return HeartbeatRejection(
                type="unauthorized_attempt",
                message="Heartbeat attempt is not currently authorized.",
            )

        job = queue.job_by_id(attempt.job_id)
        if job is None:
            return HeartbeatRejection(
                type="parent_job_missing",
                message="Authorized heartbeat attempt has no parent Queue job.",
            )

        logical = job.logical
        if type(logical) is _TrainingJob or type(logical) is _EvaluationJob:
            study_run_id = logical.coordinate.study_run
        else:
            return HeartbeatRejection(
                type="parent_job_inconsistent",
                message="Authorized heartbeat attempt has an invalid parent job kind.",
            )
        ...
        try:
            ...
            return HeartbeatRejection(
                type="parent_study_missing",
                message="Authorized heartbeat attempt has no canonical parent Study Run.",
            )
        if study_run.id != study_run_id:
            return HeartbeatRejection(
                type="parent_study_inconsistent",
                message="Canonical parent Study Run identity disagrees with Queue lineage.",
            )

        if study_run.status in {StudyRunStatus.RUNNING, StudyRunStatus.CANCELLED}:
            updated = queue.heartbeat_attempt(
                attempt.attempt_id,
                request.lease_token,
                accepted_at=accepted_at,
            ...
            if (
                updated.attempt_id != attempt.attempt_id
                or updated.job_id != attempt.job_id
                or updated.lease_token != request.lease_token
                or updated.finished_at is not None
            ):
                return HeartbeatRejection(
                    type="heartbeat_authority_changed",
                    message="Heartbeat update did not preserve the authorized open attempt.",
                )
            return HeartbeatAccepted(
                lease_until=updated.lease_until,
                cancel_requested=study_run.status is StudyRunStatus.CANCELLED,
            )
        ...
        if study_run.status in {
            ...
            return HeartbeatRejection(
                type="parent_study_terminal",
                message="Authorized attempt belongs to a terminal non-cancelled Study Run.",
            )

        return HeartbeatRejection(
            type="parent_study_inconsistent",
            message="Canonical parent Study Run has an unsupported status.",
        )
