...
const DATABASE_NAME = 'mjtensu-capture-dataset';
const DATABASE_VERSION = 1;
const STORE_NAME = 'pending-captures';

export async function putPendingCapture(capture: PendingCapture): Promise<void> {
...
export async function listPendingCaptures(): Promise<PendingCapture[]> {
  const database = await openDatabase();
  try {
    return await transactionRequest<PendingCapture[]>(
      database,
      'readonly',
      (store) => store.getAll() as IDBRequest<PendingCapture[]>,
    );
  } finally {
    database.close();
  }
