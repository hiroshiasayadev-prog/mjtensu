...
- Follow-up iPhone rounded-corner protection on 2026-08-28 updates the shared `MobileScoringPageShell` / persistent bottom bar with horizontal safe-area padding for the app bar, scroll content, and fixed actions. This is intentionally shared with I10 Conditions rather than duplicated in Result-specific layout code.
- Verification after the horizontal safe-area correction: `npx vitest run test/conditions-page.test.tsx test/result-page.test.tsx test/shell-routing.test.tsx` — **PASS**, 3/3 files and 54/54 tests; `npm run typecheck` — **PASS**.
